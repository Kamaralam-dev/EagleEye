...
namespace Codaxy.WkHtmlToPdf
{
    ...
    public class PdfConvert
    {
        ...
        private static string GetWkhtmlToPdfExeLocation()
        {
            ...
            string programFilesPath = HttpContext.Current.Server.MapPath("~/bin");// System.Environment.GetEnvironmentVariable("ProgramFiles");
            ...
            filePath = Path.Combine(programFilesPath, @"wkhtmltopdf.exe");
            if (File.Exists(filePath))
                return filePath;
        ...
        public static void ConvertHtmlToPdf(PdfDocument document, PdfConvertEnvironment environment, PdfOutput woutput)
        {
            ...
            //paramsBuilder.Append("--viewport-size 1280x1024 --orientation Landscape ");
            ...
            if (!string.IsNullOrEmpty(document.ViewPort))
                paramsBuilder.AppendFormat("--viewport-size {0} ", document.ViewPort);
